...
3.  Next we take a look a SV_RunGameFrame();

    we go into quake2/sv_main.c to checkout the SV_RunGameFrame(){}

    as mentioned earlier the server loads in game dll and runs code there 
    so when it gets to ge->RunFrame(); that will go into the our game code 
...
4.  that goes into game.dll/g_main.c
    ...
    -   note that we do an if check to see if entity is inuse 
